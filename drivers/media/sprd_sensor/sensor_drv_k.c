...
#elif defined(CONFIG_MACH_YOUNG23GDTV) || defined(CONFIG_MACH_J13G) || defined(CONFIG_MACH_VIVALTO3MVE3G_LTN)
...
#if  defined (CONFIG_MACH_CORE3_W)|| defined(CONFIG_MACH_J13G)
	if (SENSOR_MAIN == _sensor_K_get_curId() && dvdd_val == SENSOR_VDD_1200MV)
	{
...
#if  defined (CONFIG_MACH_GRANDNEOVE3G) && !defined( CONFIG_SENSOR_OUTPUT_RAW)
...
#if  defined (CONFIG_MACH_VIVALTO5MVE3G) && !defined( CONFIG_SENSOR_OUTPUT_RAW) && !defined(CONFIG_MACH_VIVALTO3MVE3G_LTN)
	if (SENSOR_MAIN == _sensor_K_get_curId())
	{
		if (SENSOR_VDD_CLOSED == dvdd_val)
		{
...
int sensor_k_sensor_sel(uint32_t sensor_id)
{
	...
	if(SENSOR_MAIN == sensor_id)
	{
		...
		SENSOR_PRINT_HIGH("Sensor : sensor_k_sensor_sel : Main camera I2C ID(%d), I2C address = %x OK\n", sensor_id, s_p_sensor_mod->sensor_i2c_driver.address_list);
	}
	else
	{
		s_p_sensor_mod->sensor_i2c_driver.driver.name = SENSOR_SUB_I2C_NAME;
		s_p_sensor_mod->sensor_i2c_driver.address_list = &c_sensor_main_default_addr_list[1];
		SENSOR_PRINT_HIGH("Sensor : sensor_k_sensor_sel : Front camera I2C ID(%d) I2C address = %x OK\n", sensor_id, s_p_sensor_mod->sensor_i2c_driver.address_list);
	}
#endif
...
LOCAL int _Sensor_K_WriteReg(SENSOR_REG_BITS_T_PTR pReg)
{
	...
	else
	{
		...
		if(s_p_sensor_mod->cur_i2c_client->addr == 0x0028)
			delay_value = 0xdddd;
		else
			delay_value = 0xffff;
#else 
	...
	SENSOR_PRINT_ERR("_Sensor_K_WriteReg delay_value = %x, subaddr = %x\n",delay_value,subaddr);

	if (0xdddd != subaddr)
	{
	SENSOR_PRINT_ERR("_Sensor_K_WriteReg I2C Addr = %x, addr = %x, value = %x, bit = %d\n",
...
int sensor_k_probe(struct platform_device *pdev)
{
	...
	{
		gpio_direction_output(s_p_sensor_mod->pin_main_reset, 0);
	}
	...
	if (s_p_sensor_mod->pin_sub_reset != s_p_sensor_mod->pin_main_reset) 
	{
		...
		{
			gpio_direction_output(s_p_sensor_mod->pin_sub_reset, 0);
		}
	}
...
#if defined (CONFIG_MACH_VIVALTO5MVE3G) && !defined( CONFIG_SENSOR_OUTPUT_RAW)
...
#if !defined(CONFIG_MACH_YOUNG23GDTV) && !defined(CONFIG_MACH_J13G) && !defined(CONFIG_MACH_GOYAVE3G) && !defined(CONFIG_MACH_GOYAVEWIFI)
...
#endif // !defined(CONFIG_MACH_YOUNG23GDTV) && !defined(CONFIG_MACH_J13G) && !defined(CONFIG_MACH_GOYAVE3G)
...
int __init sensor_k_init(void)
{
	...
	{
		sci_glb_write(CTL_PIN_BASE + camera_stby_rst_pinmap[i].reg, camera_stby_rst_pinmap[i].val, 0xffffffff);
		printk("sensor_k_init : Pinmap(%x) for camera is written\n", CTL_PIN_BASE + camera_stby_rst_pinmap[i].reg);
	}
#endif
