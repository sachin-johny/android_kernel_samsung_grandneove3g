		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	ret = k2hh_do_calibrate(data, (int)enable);
	if (ret < 0)
...
static int k2hh_parse_dt(struct k2hh_p *data, struct device *dev)
{
	...
	data->acc_int1 = of_get_named_gpio_flags(node,
	...
	data->sda_gpio = of_get_named_gpio_flags(node,
		"stm,sda", 0, &flags);
	if (data->sda_gpio < 0)
		pr_info("[SENSOR]: %s - no sda_gpio\n", __func__);

	data->scl_gpio = of_get_named_gpio_flags(node,
		"stm,scl", 0, &flags);
	if (data->scl_gpio < 0)
		pr_info("[SENSOR]: %s - no scl_gpio\n", __func__);

	ret = of_property_read_u32(node, "stm,axis_map_x", &temp);
	...
	ret = of_property_read_u32(node, "stm,axis_map_y", &temp);
	...
	ret = of_property_read_u32(node, "stm,axis_map_z", &temp);
	...
	ret = of_property_read_u32(node, "stm,negate_x", &temp);
	...
	ret = of_property_read_u32(node, "stm,negate_y", &temp);
	...
	ret = of_property_read_u32(node, "stm,negate_z", &temp);
