...
#include <linux/input/bt532_ts_spec.h>
...
/*---------------------------------------------------------------------*/
//SDND
#define SEC_SDND_N_COUNT_00			20
#define SEC_SDND_U_COUNT_00			1
#define SEC_SDND_FREQUENCY_00		77 /* 307khz */

//PDND
#define SEC_PDND_N_COUNT_00			16
#define SEC_PDND_U_COUNT_00			14
#define SEC_PDND_FREQUENCY_00		79

/*---------------------------------------------------------------------*/
/* G1F */
/*---------------------------------------------------------------------*/
//SDND
#define SEC_SDND_N_COUNT_03			18
#define SEC_SDND_U_COUNT_03			2
#define SEC_SDND_FREQUENCY_03		59

//PDND
#define SEC_PDND_N_COUNT_03			14
#define SEC_PDND_U_COUNT_03			14
#define SEC_PDND_FREQUENCY_03		59
/*---------------------------------------------------------------------*/
#else
//SDND
#define SEC_SDND_N_COUNT				20
#define SEC_SDND_U_COUNT				1
#define SEC_SDND_FREQUENCY			77 /* 307khz */
...
static bool recheck_reference_data(void);
...
static int bt532_power(int on)
{
	...
	if (touch_regulator == NULL) {
		touch_regulator = regulator_get(NULL, "vddsim2");
...
#endif
//	b_recheck_cal_data = recheck_reference_data();
//	pr_info("recheck_reference_data result : %d \n", b_recheck_cal_data);
...
static bool ts_set_touchmode(u16 value)
{
	...
	if(value == TOUCH_DND_MODE) { //SDND mode
		...
		{   //G1F
			N_cnt = SEC_SDND_N_COUNT_03;
			U_cnt = SEC_SDND_U_COUNT_03;
			freq = SEC_SDND_FREQUENCY_03;
		}
#else
		...
		{   //G1F
			N_cnt = SEC_PDND_N_COUNT_03;
			U_cnt = SEC_PDND_U_COUNT_03;
			freq = SEC_PDND_FREQUENCY_03;
		}
#else
		N_cnt = SEC_SDND_N_COUNT;
		U_cnt = SEC_SDND_U_COUNT;
...
static bool get_raw_data_in_probe(struct bt532_ts_info *info, u8 *buff, int skip_cnt)
{
	struct i2c_client *client = info->client;
	struct bt532_ts_platform_data *pdata = info->pdata;
	u32 total_node = info->cap_info.total_node_num;
...
static bool recheck_reference_data(void)
{
	...
	if (misc_info->work_state != PROBE)
		return false;
	if (read_data(misc_info->client, BT532_TOTAL_NUMBER_OF_X,(u8 *)&misc_info->cap_info.x_node_num, 2) < 0)
		if (read_data(misc_info->client, BT532_TOTAL_NUMBER_OF_X,(u8 *)&misc_info->cap_info.x_node_num, 2) < 0);

	if (read_data(misc_info->client, BT532_TOTAL_NUMBER_OF_Y,(u8 *)&misc_info->cap_info.y_node_num, 2) < 0)
		if (read_data(misc_info->client, BT532_TOTAL_NUMBER_OF_Y,(u8 *)&misc_info->cap_info.y_node_num, 2) < 0);

	misc_info->cap_info.total_node_num = misc_info->cap_info.x_node_num * misc_info->cap_info.y_node_num;

	if (write_reg(misc_info->client, BT532_TOUCH_MODE,	8) != I2C_SUCCESS)
		printk(KERN_INFO "[zinitix_touch] TEST Mode : "
				"Fail to set ZINITX_TOUCH_MODE %d.\r\n", 8);
	...
	get_raw_data_in_probe(misc_info, (u8 *)misc_info->ref_data, 5);

	if (write_reg(misc_info->client, BT532_TOUCH_MODE,	TOUCH_POINT_MODE) != I2C_SUCCESS)
		if (write_reg(misc_info->client, BT532_TOUCH_MODE,	TOUCH_POINT_MODE) != I2C_SUCCESS)
			if (write_reg(misc_info->client, BT532_TOUCH_MODE,	TOUCH_POINT_MODE) != I2C_SUCCESS);

	min = misc_info->ref_data[0];
	max = misc_info->ref_data[0];

	for(i = 0; i < misc_info->cap_info.x_node_num - 1; i++)
	{
		for(j = 0; j < misc_info->cap_info.y_node_num; j++)
		{
			pr_info("ref_data : %d ",misc_info->ref_data[i * misc_info->cap_info.y_node_num + j]);

			if (misc_info->ref_data[i * misc_info->cap_info.y_node_num + j] < min &&
				misc_info->ref_data[i * misc_info->cap_info.y_node_num + j] != 0)
				min = misc_info->ref_data[i * misc_info->cap_info.y_node_num + j];

			if (misc_info->ref_data[i * misc_info->cap_info.y_node_num + j] > max)
				max = misc_info->ref_data[i * misc_info->cap_info.y_node_num + j];

		}
		pr_info("\n");
	}

	printk("NVM data max = %d, min = %d\n", max, min);
		...
		const struct i2c_device_id *i2c_id)
{
	...
	ret = gpio_request(pdata->gpio_int, "bt532_int");
	if (ret < 0) {
		pr_err("%s: Request GPIO failed, gpio %d (%d)\n", BT532_TS_DEVICE,
			pdata->gpio_int, ret);
		goto err_gpio_alloc;
